...
- **Superseded during execution (recorded here, task text left as written):** Task 4's framing above -- "bring the Windows CI leg to parity with ubuntu" -- was overridden while this plan was carried out. The Windows leg (`windows-validation`) was kept a deliberate platform-sensitive subset rather than turned into a second full gate; the four suites actually added (`test:commands-expansion`, `test:web-dashboard`, `test:simulate-e2e`, `test:autonomous-runner`) were selected on that narrower criterion, not on matching ubuntu's suite list. The rationale lives in commit `4c5af7b`'s body and in a gitignored decision ledger; this plan is left unedited as the historical record of what was originally proposed, with this note as the pointer to what actually shipped and why.
