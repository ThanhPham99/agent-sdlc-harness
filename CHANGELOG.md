...
- `putArtifact` writes metadata before the object. The two writes cannot be made atomic, so the choice is which half a crash leaves behind, and only one is cheap and self-repairing: a few hundred bytes of JSON that the next identical put restores, rather than an arbitrarily large object nothing rewrites.

### Added
- `agent-sdlc migrate` converts a v1 tree to v2. It copies the whole tree to `.agent-sdlc/.backup-v1-<timestamp>/` before touching anything, **moves** rather than copies (a copy would leave the legacy directories in place and the project would report `compatible: false` forever), never overwrites an existing v2 path, and rewrites no file content — every operation is a rename. It reports `INCOMPLETE` rather than success if anything is left behind: a populated legacy directory, a filename v2 cannot address, or a move that failed. An incomplete migration is recorded in `LAYOUT.json` so a later run resumes it; a healthy v2 project is left alone even when it holds files the migrator does not recognise. Verified against a real 25-run, 378-artifact tree.
- `compat-check` reports the layout version, and answers it before anything about state metadata. A v1 tree read by a v2 harness has no visible runs, artifacts or history, and was previously reported `MIGRATION_AVAILABLE, compatible: true` — "your state metadata is slightly behind" — for a project whose entire history was unreachable. Detection recognises the v1 *shape* by its own markers rather than trusting the version stamp, because `init` writes the stamp unconditionally and would otherwise silence the guard permanently on a tree whose data is still unreachable.

### Fixed
- `VERIFY` now runs the project's test suite over the integrated result of every `DONE` task instead of asserting `targeted_verification_pass`. The autonomous runner's `VERIFY` stage previously invoked only the secret and SAST scans and passed the gate token inline (`internal: true` bypasses `guardEvidenceAuthority`), so a regression that only appears once task workspaces are merged reached `RELEASE` unseen — and a project with no CI configured never ran a full suite at all. `runIntegratedVerification` prefers `test.run_full`, falls back to the plan's declared targeted selectors, pauses at `GATE_2_ESCALATION_BLOCKER` on failure, and reports `UNAVAILABLE` rather than passing when no test command is configured. `runtime/tools.mjs` now records the gate token for `test.run_full` as well, so the token is always produced by a run.
- The quality gate enforces `policies/coding-standards.json` deterministically. `runtime/coding-standards-linter.mjs` existed but nothing outside its own test called it; clean-code compliance rested entirely on prompt text in `implementation.md` and `code-review.md`. `codingStandardsFindings` now lints the task's changed files (never the whole repository) at `QUALITY_REVIEW` and merges the violations into the review as findings carrying `file:line` evidence; a `BLOCKING` violation overrides an `ACCEPTED` verdict to `CHANGES_REQUIRED`, which classifies as `QUALITY_BLOCKER` and routes to recovery. Non-blocking violations are capped at `MAX_REPORTED_STANDARDS_NITS` so a noisy file cannot bury the reviewer's own findings.

### Added
- `runtime/task-reviewer.mjs` spawns a reviewer agent for each of the two task review contracts, so `agent-sdlc auto` produces real reviews instead of stopping at `REVIEW` for want of one. The reviewer runs as its own process through the existing provider layer, is shown the task specification and the diff and nothing the writer said about its own work, and is handed the published JSON schema as its output contract. Three properties make it a review rather than a formality: independence is established by what the prompt omits; every binding (`schema`, ids, `attempt`, `diff_hash`) is written by the harness afterwards and never read back from the model, so a reviewer cannot pass or fail on bookkeeping; and every failure mode — no provider, host failure, timeout, unparseable output, a verdict outside the contract — returns `UNAVAILABLE`, never a clean review. `agent-sdlc auto --no-reviewer` disables spawning, which stops the run at `REVIEW` rather than letting it through.
- `standards_audit` on every code-quality review: whether the audit ran (`RAN`, `DISABLED`, `NO_LINTABLE_FILES`, `SKIPPED`, `ERROR`), how many files it read, which policy applied, and why it did not run. An audit that found nothing and an audit that never happened otherwise produce the same empty finding list, which is the same class of clean-looking artifact proving nothing that these fixes exist to remove. A failed audit is recorded rather than blocking — a broken linter must not strand a task.
- `coding_standards` in `.agent-sdlc/project.json` (`enabled`, `policy_path`) lets a project choose or disable the standards policy. The harness policy is opinionated (snake_case properties, no `var`, no `any`) and applying it unasked to a repository with different conventions would block every task there. With nothing configured, a project-level `policies/coding-standards.json` takes precedence over the harness's.
- `findings[].category` on a standards violation is `MAINTAINABILITY`, an actual member of the enum in `CodeQualityReview.schema.json`. An invented category passed the hand-rolled checks in `task-review.mjs` and would have been rejected by the published contract — which now matters concretely, because that schema is handed to the spawned reviewer as `--json-schema`. The schemas also gained `rule_id` on quality findings, plus `standards_audit` and `generated_by`, all of which the harness was already writing into documents declared `additionalProperties: false`.
- The autonomous runner no longer rubber-stamps its own reviews. It constructed `COMPLIANT`/`ACCEPTED` documents with empty findings whatever the diff contained, so `sdlc-orchestrator`'s "a clean spec-compliance review and a clean code-quality review" was satisfied by nothing having looked. `runAutoTaskLoop` and `runAutoPipeline` accept a `reviewerCallback`; without one the placeholder reviews are stamped `AUTO_RUNNER_STUB` on the task and `REVIEW` pauses at `GATE_2_ESCALATION_BLOCKER` naming the unreviewed tasks rather than recording `required_reviews_resolved`. Recording a real review through `recordTaskReview` clears the marker.
