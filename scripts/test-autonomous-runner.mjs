...
await test('spec-review-stub-carries-independence-and-does-not-crash-on-strict-tasks',async ()=>{
  const d=fixture('auto-strict-stub-review');
  fs.mkdirSync(path.join(d,'src'),{recursive:true});
  fs.writeFileSync(path.join(d,'src','index.js'),'export const a = 1;\n');
  execFileSync('git',['add','.'],{cwd:d});
  execFileSync('git',['-c','user.email=test@test.local','-c','user.name=test','commit','-qm','init','--allow-empty'],{cwd:d});

  const r=route(ROOT,'Emergency hotfix for auth crash');
  const run=newRun(ROOT,d,{objective:'Emergency hotfix for auth crash',route:r});
  ...
  const workerCallback=(t)=>{
    ...
    const targetDir=ws?.root||d;
    fs.mkdirSync(path.join(targetDir,'src'),{recursive:true});
    fs.writeFileSync(path.join(targetDir,'src','index.js'),'export const a = 2;\n');
  };

  // No reviewer supplied or spawned -> runner generates stubs
  const res=runAutoPipeline(ROOT,d,run,{
    workerCallback,
    spawnReviewer:false,
    skipCiCheck:true
  ...
  // Task should have advanced through IMPLEMENT without crashing on INDEPENDENCE_NOT_RECORDED
  // and then paused at REVIEW because reviews were generated by AUTO_REVIEW_STUB
  assert(res.status==='PAUSED','pipeline should pause');
  assert(res.current_stage==='REVIEW',`pipeline should reach REVIEW stage, got ${res.current_stage}`);
  assert(res.unreviewed_tasks?.includes('TASK-001'),'unreviewed tasks must list TASK-001');
});

await test('resuming-auto-pipeline-preserves-authored-design-decision-in-strict-workflow',async ()=>{
  const d=fixture('auto-strict-design-resume');
  fs.mkdirSync(path.join(d,'src'),{recursive:true});
  fs.writeFileSync(path.join(d,'src','index.js'),'export const db = 1;\n');
  execFileSync('git',['add','.'],{cwd:d});
  execFileSync('git',['-c','user.email=test@test.local','-c','user.name=test','commit','-qm','init','--allow-empty'],{cwd:d});

  const r=route(ROOT,'Migrate users table schema');
  let run=newRun(ROOT,d,{objective:'Migrate users table schema',route:r});

  // Step 1: Pauses at Gate 1
  let res=runAutoPipeline(ROOT,d,run,{spawnWorker:false,spawnReviewer:false});
  assert(res.status==='PAUSED'&&res.pause_gate===HUMAN_GATES.GATE_1_SCOPE_AND_ARCHITECTURE);

  // Step 2: Grant ticket
  const {grantApprovalTicket,activeCapabilities}=await import('../runtime/approvals.mjs');
  grantApprovalTicket(ROOT,d,run,{ticketId:res.approval_ticket.ticket_id,actor:'ARCHITECT',reason:'Approve design'});
  run=loadRun(d,run.run_id);

  // Step 3: Pauses with validation_errors demanding authored decision
  res=runAutoPipeline(ROOT,d,run,{spawnWorker:false,spawnReviewer:false});
  assert(res.status==='PAUSED'&&res.validation_errors);

  // Step 4: Record authored decision
  const {recordDesignDecision}=await import('../runtime/orchestrator.mjs');
  const crypto=await import('node:crypto');
  const rec=recordDesignDecision(ROOT,d,run,{
    schema:'agent-sdlc/design-decision/v1',
    decision_id:`design_${crypto.randomUUID()}`,
    objective:run.objective,
    mode:'FULL',
    decision:'Zero-downtime online migration',
    options:[
      {id:'OPTION-A',summary:'Expand and contract',benefits:['No downtime'],tradeoffs:['Multi-step']},
      {id:'OPTION-B',summary:'Lock and migrate',benefits:['Single step'],tradeoffs:['Downtime']}
    ],
    recommended_option:'OPTION-A',
    approval:{required:true,status:'APPROVED'}
  },{approvals:activeCapabilities(ROOT,run)});
  assert(rec.recorded===true,'authored design decision was recorded');

  // Step 5: Resume auto pipeline -> should advance PAST DESIGN without re-scaffolding TODO decision
  run=loadRun(d,run.run_id);
  res=runAutoPipeline(ROOT,d,run,{spawnWorker:false,spawnReviewer:false});
